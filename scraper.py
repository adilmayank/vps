...
class Scrape:
    ...
    def scrape_contact_deatils_from_html(self, path, number_of_files=1):
        ...
        def scrape_details(file=None, header=None):
            ...
            pd.DataFrame(customer_details).to_csv(f"E:/rp_scraping/test.csv",               #change test.csv to appropriate output file or rename it afterwords
                                                  header=header,
                                                  index=False,
                                                  mode="a")

        first_file = path + rf"\3023_customer_{1}.html"                                     #change 3023 to appropriate postal code during scraping
        ...
        if (number_of_files > 1):
            ...
            for file_number in range(2, number_of_files + 1):
                try:
                    subsequent_file = path + rf"\3023_customer_{file_number}.html"          #change 3023 to appropriate postal code during scraping
